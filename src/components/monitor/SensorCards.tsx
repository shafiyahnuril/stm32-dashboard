...
import { Thermometer, Ruler, Droplets } from 'lucide-react';
...
export function SensorCards() {
  ...
  const humidity = (data as any).humidity ?? 60.0;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-blue-500 to-cyan-500 p-5 rounded-2xl shadow-sm text-white flex items-center justify-between group cursor-default">
        <div className="relative z-10">
          <div className="text-xs font-medium text-white/80 uppercase tracking-wider mb-1">Humidity</div>
          <div className="text-4xl font-extrabold tracking-tight mb-1">{humidity}%</div>
          <div className="text-xs text-white/70 font-medium">Sensor Output</div>
        </div>
        <Droplets size={56} className="text-white/20 absolute right-4 group-hover:scale-110 transition-transform" />
      </div>
